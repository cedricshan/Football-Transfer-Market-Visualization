
var parseDate = d3.timeParse("%B");
const margin = { top: 20, right: 20, bottom: 40, left: 40 };
document.getElementById("csv-select").addEventListener("change", function () {
    loadAndVisualizeData(this.value);
});

function loadAndVisualizeData(filename) {

    d3.select("#chartSVG").remove();

    var svg = d3.select('body')
        .append('svg')
        .attr('id', 'chartSVG')
        .attr('height', '100%')
        .attr('width', '100%');

    d3.csv(filename)
        .then(data => {
            return data.map(d => {
                return {
                    Fee: Number(d.Fee),
                    Age: Number(d.Age)
                };
            });
        })
        .then(processedData => {
            var height = 500;
            var width = 800;
            var maxFee = d3.max(processedData, function (d) { return d.Fee; });
            var minFee = d3.min(processedData, function (d) { return d.Fee; });
            var minAge = d3.min(processedData, function (d) { return d.Age; })
            var maxAge = d3.max(processedData, function (d) { return d.Age; })
            const meanFee = d3.mean(processedData, d => d.Fee);

            // Group data by Age
            const groupedData = d3.group(processedData, d => d.Age);

            // Compute statistics for each group
            const sumstat = Array.from(d3.rollup(
                processedData,
                group => {
                    const fees = group.map(d => +d.Fee).sort(d3.ascending);

                    const q1 = d3.quantile(fees, 0.25);
                    const median = d3.quantile(fees, 0.5);
                    const q3 = d3.quantile(fees, 0.75);
                    const top = d3.quantile(fees, 0.15);
                    const bottom = d3.quantile(fees, 0.85);
                    const interQuantileRange = q3 - q1;
                    const min = 0;
                    const max = q3 + 1.5 * interQuantileRange;
                    const avg = d3.mean(fees);
                    return { q1, median, q3, interQuantileRange, min, max, avg, top, bottom };

                },
                d => d.Age
            )).map(([key, value]) => ({ key, value }));

            function selectRandomTenPoints(data) {
                const sampledData = [];
                for (const [age, values] of groupedData) {
                    // Filter out values with Fee over 10,000,000
                    const filteredValues = values.filter(d => d.Fee <= d3.mean(processedData, d => d.Fee) * 2.8);

                    const shuffled = [...filteredValues].sort(() => 0.5 - Math.random());
                    const selected = shuffled.slice(0, 10);
                    sampledData.push(...selected);
                }
                return sampledData;
            }


            const sampleDataForPlotting = selectRandomTenPoints(processedData);

            const sortedData = sumstat.sort((a, b) => d3.ascending(a.key, b.key));

            console.log(sortedData);

            console.log(maxFee);
            console.log(maxAge);
            console.log(minAge);

            const x = d3.scaleLinear()
                .domain([d3.min(processedData, d => d.Age) - 1, d3.max(processedData, d => d.Age) + 1])
                .range([0, width]);

            const y = d3.scaleLinear()
                .domain([d3.min(processedData, d => d.Fee), d3.mean(processedData, d => d.Fee) * 2.8])
                .range([height, 0]);


            var chartGroup = svg
                .append('g')
                .attr('transform', 'translate(100,100)');

            chartGroup.append("g")
                .attr("transform", `translate(0, ${height})`)
                .call(d3.axisBottom(x));

            chartGroup.append("g")
                .call(d3.axisLeft(y))



            chartGroup.append("text")
                .attr("transform", `translate(${width / 2}, ${height + margin.top + 20})`)
                .style("text-anchor", "middle")
                .text("Age");

            chartGroup.append("text")
                .attr("transform", "rotate(-90)")
                .attr("y", 0 - margin.left - 50)
                .attr("x", 0 - (height / 2))
                .attr("dy", "1em")
                .style("text-anchor", "middle")
                .text("Fee (euro)");

            chartGroup
                .selectAll("vertLines")
                .data(sumstat)
                .enter()
                .append("line")
                .attr("x1", function (d) { return (x(d.key)) })
                .attr("x2", function (d) { return (x(d.key)) })
                .attr("y1", function (d) { return (y(d.value.bottom)) })
                .attr("y2", function (d) { return (y(d.value.top)) })
                .attr("stroke", "black")
                .style("width", 40)



            chartGroup
                .selectAll("Lines")
                .data(sumstat)
                .enter()
                .append("line")
                .attr("x1", function (d) { return (x(d.key)) })
                .attr("x2", function (d) { return (x(d.key)) })
                .attr("y1", function (d) { return (y(d.value.avg)) })
                .attr("y2", function (d) { return (y(d.value.avg)) })
                .attr("stroke", "black")
                .style("width", 40)

            var tooltip = d3.select("body").append("div")
                .attr("class", "tooltip")
                .style("width", "text.length + 'px'")
                .style("height", "35px")
                .style("text-align", "left")

            var boxWidth = 20
            var highlightedBox = chartGroup.select(null);
            chartGroup
                .selectAll("boxes")
                .data(sumstat)
                .enter()
                .append("rect")
                .attr("x", function (d) { return (x(d.key) - boxWidth / 2) })
                .attr("y", function (d) { return (y(d.value.q3)) })
                .attr("height", function (d) { return (y(d.value.q1) - y(d.value.q3)) })
                .attr("width", boxWidth)
                .attr("stroke", "black")
                .style("fill", "#69b3a2")
                .on("mouseover", function (d) {
                    highlightedBox.style("fill", "#69b3a2")
                    highlightedBox = d3.select(this);
                    highlightedBox.style("fill", "steelblue");
                })
                .on("mouseover", function (event, d) {
                    highlightedBox.style("fill", "#69b3a2");

                    highlightedBox = d3.select(this);
                    highlightedBox.style("fill", "steelblue");

                    tooltip.transition()
                        .duration(200)
                        .style("opacity", 0.8);
                    console.log(d.Fee)

                    tooltip.html(`Age: ${d.key}<br>Median Fee: €${Math.round(d.value.median) / 1000000}m`)
                        .style("left", (event.pageX + 5) + "px")
                        .style("top", (event.pageY - 28) + "px");
                })
                .on("mouseout", function () {
                    tooltip.transition()
                        .duration(500)
                        .style("opacity", 0);
                    highlightedBox.style("fill", "#69b3a2")
                });

            // Show the median
            chartGroup
                .selectAll("medianLines")
                .data(sumstat)
                .enter()
                .append("line")
                .attr("x1", function (d) { return (x(d.key) - boxWidth / 2) })
                .attr("x2", function (d) { return (x(d.key) + boxWidth / 2) })
                .attr("y1", function (d) { return (y(d.value.median)) })
                .attr("y2", function (d) { return (y(d.value.median)) })
                .attr("stroke", "black")
                .style("width", 80)

            var avgline = chartGroup.append('g')
                .attr('class', 'avgline')
                .attr('transform', 'translate(0,0)');

            var line = d3.line()
                .x(function (d) { return x(d.key) })
                .y(function (d) { return y(d.value.avg) });

            chartGroup.append('path')
                .attr('d', line(sortedData))
                .attr('stroke', '#FF7F50')
                .attr('stroke-width', 2)
                .attr('fill', 'none');

            var jitterWidth = 10
            chartGroup
                .selectAll("indPoints")
                .data(sampleDataForPlotting)
                .enter()
                .append("circle")
                .attr("cx", function (d) { return (x(d.Age) - jitterWidth / 2 + Math.random() * jitterWidth) })
                .attr("cy", function (d) { return (y(d.Fee)) })
                .attr("r", 4)
                .style("fill", "white")
                .attr("stroke", "black")
                .on("mouseover", function (event, d) {
                    tooltip.transition()
                        .duration(200)
                        .style("opacity", 0.8);
                    console.log(d.Fee)
                    tooltip.html(`Age: ${d.Age}<br>Fee: €${Math.round(d.Fee) / 1000000}m`)
                        .style("left", (event.pageX + 5) + "px")
                        .style("top", (event.pageY - 28) + "px");
                })
                .on("mouseout", function (event, d) {
                    tooltip.transition()
                        .duration(500)
                        .style("opacity", 0);
                });

            /*
                    chartGroup.append("text")
                        .attr("transform", `translate(${width / 2}, ${-10})`) 
                        .style("text-anchor", "middle")
                        .style("font-size", "16px")
                        .style("font-weight", "bold")
                        .text("Box Plot of Age vs. Transfer Fee");
            */

        })
        .catch(error => {
            console.error("Error loading or processing the CSV:", error);
        });
}
loadAndVisualizeData(document.getElementById("csv-select").value);
